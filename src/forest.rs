...
impl<G: GrammarReflector, I: Input> ParseForest<G, I> {
    ...
    pub fn dump_graphviz(&self, root: Option<Node<G>>, out: &mut dyn Write) -> io::Result<()> {
...
pub mod dynamic {
    ...
    #[derive(Clone)]
    struct ExpandedTree<G: GrammarReflector> {
        node: Node<G>,
        kind: ExpandedTreeKind<G>,
    }

    #[derive(Clone)]
    enum ExpandedTreeKind<G: GrammarReflector> {
        Leaf,
        Or(G::NodeKind, Rc<ExpandedTree<G>>),
        Opt(Option<Rc<ExpandedTree<G>>>),
        Concat([Rc<ExpandedTree<G>>; 2]),
    }

    impl<G: GrammarReflector> ExpandedTree<G> {
        fn one_from_node<I>(
        ...
        fn all_from_node<I>(forest: &ParseForest<G, I>, node: Node<G>) -> Vec<Rc<Self>>
        where
            I: Input,
        {
    ...
    impl<Pat: Eq + Hash + fmt::Debug, I: Input> OwnedHandle<'_, Pat, I> {
        ...
        pub fn with<R>(&self, f: impl FnOnce(Handle<'_, '_, Pat, I>) -> R) -> R {
            self.forest_and_node.unpack_ref(|_, forest_and_node| {
                let (ref forest, node) = *forest_and_node;
                f(Handle {
                    forest,
    ...
    pub struct Handle<'a, 'b, Pat: Eq + Hash + fmt::Debug, I: Input> {
        pub forest: &'a ParseForest<CxAndGrammar<'b, Pat>, I>,
        pub node: Node<CxAndGrammar<'b, Pat>>,
        pub fields: Option<IFields>,
        // FIXME(eddyb) support an arbitrary number of disambiguations here
        disambiguator: Option<(Node<CxAndGrammar<'b, Pat>>, usize)>,
    }

    impl<Pat: Eq + Hash + fmt::Debug, I: Input> Copy for Handle<'_, '_, Pat, I> {}

    impl<Pat: Eq + Hash + fmt::Debug, I: Input> Clone for Handle<'_, '_, Pat, I> {
        fn clone(&self) -> Self {
            *self
        }
    }

    impl<Pat: Eq + Hash + fmt::Debug, I: Input> fmt::Debug for Handle<'_, '_, Pat, I> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    impl<'a, 'b, Pat: Eq + Hash + fmt::Debug, I: Input> Handle<'a, 'b, Pat, I> {
                ...
                Rule::Concat([left_rule, right_rule]) => {
                    let split = match self.disambiguator {
                        Some((dis_node, dis_split)) if dis_node == node => {
                            let (left, right) = node.range.split_at(dis_split);
                            Ok((
                                Node {
                                    kind: left_rule,
                                    range: left,
        ...
        pub fn as_list(mut self) -> ListHandle<'a, 'b, Pat, I> {
        ...
        fn all_lists(mut self) -> impl Iterator<Item = ListHandle<'a, 'b, Pat, I>> {
            assert_eq!(self.fields, None);
    ...
    pub struct ListHandle<'a, 'b, Pat: Eq + Hash + fmt::Debug, I: Input> {
        pub forest: &'a ParseForest<CxAndGrammar<'b, Pat>, I>,
        tree: Rc<ExpandedTree<CxAndGrammar<'b, Pat>>>,
    }

    impl<Pat: Eq + Hash + fmt::Debug, I: Input> Clone for ListHandle<'_, '_, Pat, I> {
        fn clone(&self) -> Self {
            ListHandle {
                forest: self.forest,
    ...
    impl<'a, 'b, Pat: Eq + Hash + fmt::Debug, I: Input> Iterator for ListHandle<'a, 'b, Pat, I> {
        type Item = Result<Handle<'a, 'b, Pat, I>, Ambiguity<Handle<'a, 'b, Pat, I>>>;
    ...
    impl<Pat: Eq + Hash + fmt::Debug, I: Input> fmt::Debug for ListHandle<'_, '_, Pat, I> {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            struct Spread<T>(T);
